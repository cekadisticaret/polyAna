...
POS_SIZE_PCT     = 0.10
TRAIL_ATR_MULT   = 2.0      # 5m için daha geniş trailing
...
def analyze(symbol, interval="1m"):
    ...
    # 1m'de 30 bar = 30 dk referans — 15m sistemindeki 12 bar (3h)'a oran olarak daha güçlü filtre
    vol_confirm = (volumes[-2] > max(volumes[-30:-2]) * 1.2
                   if len(volumes) >= 30 else False)

    strong_buy = (
        not is_hot_vol and trend_up and price > ema21 and
        rsi_val > 58 and rsi_rising and
        vol_spike and vol_confirm and adx_val > ADX_MIN and macd_val > sig_val and is_not_far
    )
    strong_sell = (
        not is_hot_vol and trend_down and price < ema21 and
...
def run_scan():
    ...
    for pos in list(data["open"]):
        ...
        # ── TP Aşıldı → Trail Sıkılaştır + TP'yi Uzat ──
        if pos["symbol"] in open_set:
            tp_hit = (b_high >= pos["tp"]) if pos["direction"] == "LONG" else (b_low <= pos["tp"])
            if tp_hit:
                new_mult = max(0.4, trail_mult - 0.3)
                pos["trail_mult"] = new_mult
                if pos["direction"] == "LONG":
                    pos["tp"]  = round(b_high + entry_atr * TAKE_ATR_MULT, 6)
                    tight_sl   = round(b_high - entry_atr * new_mult, 6)
                    if tight_sl > pos["sl"]:
                        pos["sl"] = tight_sl
                else:
                    pos["tp"]  = round(b_low - entry_atr * TAKE_ATR_MULT, 6)
                    tight_sl   = round(b_low + entry_atr * new_mult, 6)
                    if tight_sl < pos["sl"]:
                        pos["sl"] = tight_sl
                dir_e = "📈" if pos["direction"] == "LONG" else "📉"
                print(f"  🎯 TP AŞILDI — devam: {pos['symbol']} | trail → {new_mult}×ATR | yeni TP: {pos['tp']}")
                tg_send(
                    f"🟢🎯 <b>TP Aşıldı — Devam Ediyoruz!</b>\n"
                    f"{dir_e} {pos['direction']} | <b>{pos['symbol']}</b>\n"
                    f"━━━━━━━━━━━━━━\n"
                    f"🛡 SL sıkılaştı : <b><code>{pos['sl']}</code></b>  ({new_mult}×ATR)\n"
                    f"🎯 Yeni TP      : <code>{pos['tp']}</code>\n"
                    f"🟢 Trend devam ediyor, pozisyon korunuyor."
                )
