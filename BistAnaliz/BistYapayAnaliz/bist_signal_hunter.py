...
Konum: BistAnaliz/BistYapayAnaliz/ — BistAnaliz/bist_scanner.py; Telegram: kök telegram_config.
...
try:
    ...
    CHAT_ID = _tg.CHAT_ID
    _sig = getattr(_tg, "CHAT_ID_BIST_SIGNAL", "") or ""
    TG_CHAT_ID = (_sig.strip() if _sig else "") or CHAT_ID
except ImportError:
    BOT_TOKEN = ""
    CHAT_ID = ""
    TG_CHAT_ID = ""
...
def run_example_notify():
    """Seans/tarama yok; gerçek GÜÇLÜ AL mesaj formatında tek örnek bildirim."""
    fake = {
        "ticker": "RYGYO",
        "interval": "1h",
        "price": 33.70,
    ...
    msg = build_message(fake, repeat_n=3)
    msg += "\n\n<i>— örnek / test mesajı (bist_signal_hunter.py --test) —</i>"
    tg_send(msg)
