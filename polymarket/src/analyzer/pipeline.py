...
from src.analyzer.poly_bridge import run_poly_hourly_predictions, Prediction
...
def _business_day_date_tr(slot_at: datetime) -> str:
    ...
    TR takvim günü — 00:00 TR sıfırlanma noktası.
    """
    return slot_at.astimezone(TR).date().isoformat()
...
def _telegram_summary_trade_date(tr_now: datetime, db_trade_date_et: str) -> str:
    ...
    00:00 TR sıfırlanmasıyla DB trade_date_et takvim günüyle örtüşür;
    00:00–00:59 arası özet hâlâ kapanan günü gösterir (00:05 bildirimi boş görünmesin).
    """
    if tr_now.hour == 0:
...
def run_hourly_cycle_resolve_summary() -> None:
    ...
    for row in fetch_pending_trades():
        ...
        if not actual:
            # Gamma API resolved/closed market'ı artık döndürmüyor olabilir.
            # Doğrudan Binance 1H mumuyla çözüm dene (mum kapanmışsa).
            actual = resolve_outcome_from_binance(slug)
            if not actual:
                logger.info(
                    "Çözüm henüz yok — Gamma boş, Binance muması da hazır değil: %s",
                    slug,
                )
                continue
            logger.info(
                "Gamma boş; Binance fallback ile çözüldü %s %s → %s",
                coin, slug, actual,
            )
