...
HOURLY_UP_DOWN_COINS = ("bitcoin", "ethereum", "solana")
...
# ---------------------------------------------------------------------------
# Binance 1H kline tabanlı fallback çözüm
# ---------------------------------------------------------------------------
...
_SLUG_RE = re.compile(
    r"^(?P<coin>bitcoin|ethereum|solana)-up-or-down-"
    r"(?P<month>[a-z]+)-(?P<day>\d+)-(?P<year>\d{4})-"
    r"(?P<hour>\d+)(?P<ampm>am|pm)-et$"
)


def _slug_to_et_start(slug: str) -> datetime | None:
    """Slug'dan ET saatinin başlangıç anını (timezone-aware) döndürür."""
    ...
    try:
        month_num = month_names.index(m.group("month"))
    except ValueError:
        return None
    if month_num == 0:
        return None
    hour_raw = int(m.group("hour"))
    ampm = m.group("ampm")
    if ampm == "am":
        hour_et = 0 if hour_raw == 12 else hour_raw
    else:
        hour_et = 12 if hour_raw == 12 else hour_raw + 12
    try:
        naive = datetime(int(m.group("year")), month_num, int(m.group("day")), hour_et, 0, 0)
        return naive.replace(tzinfo=_ET)
    except (ValueError, OverflowError):
        return None


def resolve_outcome_from_binance(slug: str) -> str | None:
    """
    Slug'dan ilgili Binance 1H mumu okur; close >= open → UP, değilse DOWN.

    Gamma API çözüm verisini döndürmediğinde (market closed/resolved) fallback
    olarak kullanılır. Mum henüz kapanmamışsa veya veri alınamazsa None döner.
    """
    m = _SLUG_RE.match(slug.lower())
    if not m:
        logger.debug("Binance fallback: slug parse edilemedi: %s", slug)
        return None

    symbol = _COIN_TO_SYMBOL.get(m.group("coin"))
    if not symbol:
        return None

    et_start = _slug_to_et_start(slug)
    if et_start is None:
        return None

    start_utc = et_start.astimezone(timezone.utc)
    start_ms = int(start_utc.timestamp() * 1000)

    # Mum kapanış zamanı = açılış + 1 saat; kapalı olduğunu doğrula
    close_time_ms = start_ms + 3_600_000 - 1
    now_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
    if now_ms <= close_time_ms:
        logger.debug(
            "Binance fallback: mum henüz kapanmadı slug=%s close_at=%s",
            slug,
    ...
    try:
        ...
        logger.warning("Binance kline hatası slug=%s: %s", slug, e)
        return None

    if not isinstance(data, list) or not data:
        logger.warning("Binance kline boş yanıt slug=%s", slug)
        return None
    ...
    try:
        ...
        logger.warning("Binance kline parse hatası slug=%s: %s", slug, e)
        return None

    # Doğru mum mu? (başlangıç zamanı eşleşmeli, ±1 dk tolerans)
    if abs(candle_open_ms - start_ms) > 60_000:
        logger.warning(
            "Binance kline zaman uyuşmazlığı slug=%s beklenen=%d gelen=%d",
            slug, start_ms, candle_open_ms,
        )
        return None

    result = "UP" if close_px >= open_px else "DOWN"
    logger.info(
        "Binance fallback çözüm %s %s open=%.4f close=%.4f → %s",
        symbol, slug, open_px, close_px, result,
    )
    return result
