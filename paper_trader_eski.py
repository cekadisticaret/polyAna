...
from binance_config import TG_BOT_TOKEN as BOT_TOKEN, TG_CHAT_ID as CHAT_ID
...
TRADES_FILE     = os.path.join(os.path.dirname(__file__), "paper_trades_eski.json")
...
MAX_OPEN        = 7        # Aynı anda max 7 pozisyon
COOLDOWN_BARS   = 4        # Kapanış sonrası 4 bar (60 dk) bekleme
MIN_HOLD_BARS   = 3        # Sinyal çıkışı için min 3 bar tutma
...
BULL_LONG_LEV   = 12       # Boğa: LONG kaldıraç
BULL_SHORT_LEV  = 8        # Boğa: SHORT kaldıraç
BEAR_LONG_LEV   = 8        # Ayı: LONG kaldıraç
BEAR_SHORT_LEV  = 12       # Ayı: SHORT kaldıraç

# Piyasa yönüne göre yön başına max açık pozisyon limiti
# Boğa : 7×%60≈4 LONG, 7×%30≈2 SHORT
# Ayı  : 2 LONG, 4 SHORT
# Nötr : her yönde max 4 (toplam 7'yi geçemez)
BULL_MAX_LONG   = 4
BULL_MAX_SHORT  = 2
BEAR_MAX_LONG   = 2
BEAR_MAX_SHORT  = 4
...
def analyze(symbol, interval="15m"):
    ...
    vol_spike   = volumes[-2] > vol_avg * 1.5                           # Son bar hacmi ort. x1.5
    vol_confirm = volumes[-2] > max(volumes[-12:-2]) * 1.2              # Son 12 barın maks x1.2

    dist_pct   = abs((price - ema100) / ema100) * 100
    is_not_far = dist_pct < 3.5
    ...
    strong_buy = (
        ...
        rsi_val > 55 and rsi_rising and
        macd_val > sig_val and
        adx_val > ADX_MIN and
        vol_spike and vol_confirm and
        is_not_far
    )
    strong_sell = (
        not is_hot_vol and
        trend_down and price < ema21 and
        rsi_val < 45 and rsi_falling and
        macd_val < sig_val and
        adx_val > ADX_MIN and
        vol_spike and vol_confirm and
        is_not_far
...
def open_position(data, symbol, direction, price, atr, result=None, bias="NEUTRAL"):
    ...
    # Ham SL/TP hesapla (Binance tick'e göre yuvarlanacak)
    if direction == "LONG":
        sl_raw = max(price - atr * STOP_ATR_MULT, price * (1 - MAX_SL_PCT / 100))
        tp_raw = price + atr * TAKE_ATR_MULT
    else:
        sl_raw = min(price + atr * STOP_ATR_MULT, price * (1 + MAX_SL_PCT / 100))
        tp_raw = price - atr * TAKE_ATR_MULT

    # Gerçek Binance emirleri
    try:
        avail, total = get_balance()
        desired  = max(total * POS_SIZE_PCT, 5.0)
        margin   = min(desired, avail)
        if margin < 5 or avail < 5:
            print(f"   ⚠️ {symbol}: yetersiz bakiye (avail={avail:.2f})")
            return False
        ...
        if qty * price < MIN_NOTIONAL:
            need_m   = min(max(margin, (MIN_NOTIONAL / lev) + 2), avail)
            margin   = need_m
            notional = margin * lev
            qty      = round_quantity(symbol, notional / price)
        ...
        # Gerçek fill fiyatını Binance'den çek
        fill_price = price
        try:
    ...
    sl_pct     = round(abs(fill_price - sl) / fill_price * 100, 3)
    tp_pct     = round(abs(tp - fill_price) / fill_price * 100, 3)
    win_prob, rr_ratio = calc_win_prob(result or {}, direction, sl_pct, tp_pct)
    prob_bar   = "█" * (win_prob // 10) + "░" * (10 - win_prob // 10)
    prob_emoji = "🟢" if win_prob >= 60 else "🟡" if win_prob >= 45 else "🔴"
    reasons    = build_entry_reason(result or {}, direction)
    reason_str = "\n".join(f"   • {r}" for r in reasons)
    bias_emoji = "🐂" if bias == "BULL" else "🐻" if bias == "BEAR" else "⚖️"
    bias_label = "Boğa" if bias == "BULL" else "Ayı" if bias == "BEAR" else "Nötr"

    data["open"].append({
        "symbol":       symbol,
        "direction":    direction,
        "entry_price":  fill_price,
        "size_usdt":    round(margin, 2),
        "sl":           sl,
        "tp":           tp,
        "open_time":    now_str(),
    ...
    tg_send(
        ...
        f"━━━━━━━━━━━━━━\n"
        f"{bias_emoji} <b>Piyasa: {bias_label}</b> → {lev}x kaldıraç\n"
        f"   Margin: {margin:.2f} USDT  (Max: {max_long}L / {max_short}S)\n"
        f"━━━━━━━━━━━━━━\n"
        f"📋 <b>Neden açtım?</b>\n"
        f"{reason_str}\n"
        f"━━━━━━━━━━━━━━\n"
...
def close_position(data, pos, price, reason,
                   already_closed=False, binance_net_pnl=None, binance_commission=None):
    symbol = pos["symbol"]

    # Gerçek pozisyonu Binance'de kapat
    if pos.get("real") and not already_closed:
        try:
    ...
    if binance_net_pnl is not None:
        pnl_usdt   = round(float(binance_net_pnl), 2)
        pnl_pct    = round((pnl_usdt / margin) * 100, 2) if margin else 0.0
        commission = round(float(binance_commission), 4) if binance_commission is not None \
                     else round(margin * lev * COMMISSION_PCT, 4)
    else:
        ...
        commission = round(margin * lev * COMMISSION_PCT, 4)
        pnl_usdt   = round(margin * pnl_pct / 100 - commission, 2)

    # Sermayeyi gerçek bakiyeden güncelle
    if pos.get("real"):
        try:
            _, total = get_balance()
            data["capital"] = round(total, 2)
        except Exception:
    ...
    data.setdefault("bar_counter", {})[symbol] = data.get("total_bars", 0)

    emoji     = "✅" if pnl_pct > 0 else "🔴"
    dir_emoji = "📈" if pos["direction"] == "LONG" else "📉"
    print(f"  {emoji} KAPANDI [{pos['direction']}]: {symbol} @ {price} | {'+' if pnl_pct>0 else ''}{pnl_pct}% | {reason}")
    ...
    tg_send(
        ...
        f"{dir_emoji} {pos['direction']} | <b>{symbol}</b>\n"
        f"━━━━━━━━━━━━━━\n"
...
def run_scan(symbols):
    ...
    # Gerçek Binance bakiyesi ve pozisyon sync
    avail, total = get_balance()
    data["capital"] = round(total, 2)
    binance_positions = {p["symbol"]: p for p in get_positions()}

    # Binance tarafında SL/TP ile kapanmış gerçek pozisyonları tespit et
    for pos in list(data["open"]):
        ...
        if sym not in binance_positions:
            ...
            if summary:
                exit_px, net_pnl, comm = summary
                try:
                    reason = "STOP LOSS (Binance)" if abs(exit_px - pos["sl"]) < abs(exit_px - pos["tp"]) \
                             else "TAKE PROFIT (Binance)"
                except Exception:
                    reason = "BINANCE SL/TP"
                print(f"   [binance_sync] {sym}: kapandı (net≈{net_pnl} USDT)")
                close_position(data, pos, exit_px, reason,
                               already_closed=True,
                               binance_net_pnl=net_pnl,
                               binance_commission=comm)
            else:
                close_position(data, pos, pos["entry_price"], "BINANCE SL/TP", already_closed=True)

    print(f"\n🤖 Eski Strateji — Binance Trader 15m | {now_str()}")
    print(f"   İşlem: {len(data['closed'])} | Bakiye: {avail:.2f} / {total:.2f} USDT | Açık: {len(data['open'])}")
    ...
    if is_new_15m:
        ...
        print(f"   {('🐂' if bias=='BULL' else '🐻' if bias=='BEAR' else '⚖️')} Piyasa: {bias} — {bias_desc}")
        ...
        if len(data["open"]) < MAX_OPEN:
            ...
            for symbol in symbols:
                ...
                if result["strong_buy"]:
                    if open_position(data, symbol, "LONG", result["price"], result["atr"], result, bias):
                        found += 1
                elif result["strong_sell"]:
                    if open_position(data, symbol, "SHORT", result["price"], result["atr"], result, bias):
                        found += 1
            if found == 0:
                print(f"   ℹ️  Bu turda sinyal bulunamadı.")
                # Saat başı Telegram bildirimi
                if now_utc.minute == 0:
                    tg_send(
