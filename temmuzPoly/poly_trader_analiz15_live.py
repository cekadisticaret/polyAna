#!/usr/bin/env python3
"""15. ANALİZ LIVE — gerçek Polymarket (BTC+ETH+SOL).

Sanal 15. Analiz (poly_trader_analiz15.py) yapısına DOKUNMAZ.
Aynı sinyal: analiz15_signal.resolve_live_signal (BTC→A6, ETH→A8, SOL→A2).

Env: PM_ANALIZ15_LIVE_ENABLED=true
Dashboard: analiz15_live aç/kapa + a15 miktarları
Cron: close :02 · open :05
"""
...
from poly_trader_analiz15 import (
    SYMBOLS,
    _resolve_signal,
    get_stats,
    get_symbol_stats,
...
STATE_FILE = os.path.join(_DIR, "poly_trader_analiz15_live_state.json")
HISTORY_FILE = os.path.join(_DIR, "poly_trader_analiz15_live_history.json")
HATA_FILE = os.path.join(_DIR, "analiz15_live_polyhata.json")

LABEL = "15. ANALİZ LIVE"
TRADE_AMOUNT = 16.0
_PM_LIVE = os.getenv("PM_ANALIZ15_LIVE_ENABLED", "false").lower() in ("1", "true", "yes")
_AMOUNT_SYSTEM = "a15"
...
async def run_open() -> None:
    if not _PM_LIVE:
        print(f"[{LABEL} open] PM_ANALIZ15_LIVE_ENABLED=false — atlandı")
        return
    ...
    for sym, direction, entry_price, pos, algo_name in opened:
        ...
        eng = f"  📊 {algo_name}" if algo_name else ""
        lines.append(
            f"{dir_icon} <b>{name}</b>  {direction}{eng}  giriş:{float(entry_price):.2f}\n"
