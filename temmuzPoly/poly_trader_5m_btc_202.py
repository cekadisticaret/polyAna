"""
5M 202 BTC TRADER — A1+A9 Konsensüs (Gerçek Polymarket)
==============================================
...
STATE_FILE   = os.path.join(_DIR, "poly_trader_5m_btc_202_state.json")
HISTORY_FILE = os.path.join(_DIR, "poly_trader_5m_btc_202_history.json")
WEEKLY_IMG   = "/tmp/poly_5m_btc_202_weekly.png"

SYMBOL           = "BTCUSDT"
INITIAL_BALANCE  = 0.0   # Gerçek sistem
AMOUNT_AGREE     = 10.0   # A1 + A9 hem aynı yönü söylüyor
...
def run() -> None:
    ...
    if state["open_positions"]:
        ...
        for pos in list(state["open_positions"]):
            ...
            amount   = pos.get("amount", AMOUNT_AGREE)
            to_win   = pos.get("to_win", amount * 2)
            pm_slug  = pos.get("pm_slug")
            pm_spent = pos.get("pm_spent", amount)

            # ── Polymarket gerçek sonucu sorgula ─────────────
            win = None; actual = None; pm_pnl = None; pm_note = ""
            if pm_slug:
                try:
                    req = urllib.request.Request(
                        f"{_PM_GAMMA_URL}?slug={pm_slug}", headers=_PM_HEADERS)
                    with urllib.request.urlopen(req, timeout=10) as r:
                        pm_data = json.load(r)
                    if pm_data:
                        pm_ev = pm_data[0]
    ...
    msg = (
        ...
        f"{dir_icon} <b>BTC {dir_tr}</b>  A1✅+A9({a9_score:+d}/4)✅  💵 ${amount:.2f} {price_str} → 🏆 ${to_win:.2f}\n"
