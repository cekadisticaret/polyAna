...
101'in 4-algo konsensüsü + 102 düzeltmeleri tek dosyada:
  ...
  3. MR: RSI + Bollinger
  4. Orderflow v2: CVD/OB çelişince orderbook öncelikli
  5. Momentum filtresi: son 3 kapanmış 5m mum sinyale ters ise iptal

Mevcut trader dosyalarına dokunmaz; sadece sinyal üretir.
Kullanım:
  python3 temmuzPoly/btc_5m_102_algo.py
  python3 -c "from btc_5m_102_algo import analyze; print(analyze())"
"""
...
MOMENTUM_FILTER_ENABLED = os.getenv("PM_5M_102_MOMENTUM_FILTER", "true").lower() in (
    "1", "true", "yes",
)
...
def analyze(
    ...
    *,
    ...
    try:
        ...
        print(f"[btc_5m_102_algo] Veri hatası: {e}", flush=True)
        return None
...
def format_signal(result: SignalResult | None) -> str:
    ...
    if result.direction is None:
        extra = result.skip_reason or f"konsensüs yok ({result.consensus}/4)"
        return f"Sinyal yok — {extra} | momentum={result.momentum}"
    icons = " ".join("🟢" if v > 0 else "🔴" if v < 0 else "⚪" for v in result.votes)
    return (
        f"{result.direction} ({result.consensus}/4) ${result.amount:.0f} "
