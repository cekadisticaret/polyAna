"""
Poly Sanal Trader — 32. Analiz (FeatureEngine composite_signal motoru)
Modlar:
...
Algoritma: Analiz32/ FeatureEngine v2 composite (algo kütüphanesi dokunulmaz)
Sembol: SOL only · Sanal $300 · $12 / $16 / $20 (WR'ye göre)
"""
import asyncio
import json
import os
...
STATE_FILE   = os.path.join(_DIR, "poly_trader_analiz32_state.json")
HISTORY_FILE = os.path.join(_DIR, "poly_trader_analiz32_history.json")
WEEKLY_IMG   = "/tmp/poly_weekly_heatmap_analiz32.png"
...
def run_daily() -> None:
    ...
    for msg in format_daily_history_tg(_LABEL, trades, day, now_tr):
        tg_send(msg)
    print(f"[{_LABEL} daily] {day} — {len(trades)} işlem gönderildi")
