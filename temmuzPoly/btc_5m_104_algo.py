...
Kullanım:
  python3 temmuzPoly/btc_5m_104_algo.py
  python3 -c "from btc_5m_104_algo import analyze; print(analyze().to_dict())"
"""
...
PAPER_TRADING = os.getenv("PM_PAPER_TRADING", "true").lower() in ("1", "true", "yes")
...
def fetch_klines(symbol: str = SYMBOL, interval: str = INTERVAL, limit: int = 200) -> list[dict]:
...
def get_htf_trend(symbol: str = SYMBOL) -> dict:
    try:
        htf_klines = fetch_klines(symbol, "1h", limit=100)
        ...
        print(f"[HTF] Trend alınamadı: {e}", flush=True)
        return {"direction": "NEUTRAL", "strength": 25.0, "ema20": 0, "ema50": 0}
...
def algo_a1_rsi_macd_ema(klines: list[dict]) -> tuple[int, str]:
    ...
    ema_vote = +1 if e9[-1] > e20[-1] > e50[-1] else -1 if e9[-1] < e20[-1] < e50[-1] else 0
    ...
    return vote, (
        ...
        f"MACD:{'↑' if macd_vote > 0 else '↓' if macd_vote < 0 else '→'}(h:{hist_current:+.4f})  "
        f"EMA9/20/50:{'↑' if ema_vote > 0 else '↓' if ema_vote < 0 else '→'}"
    )
...
def algo_trend(klines: list[dict]) -> tuple[int, str]:
    ...
    if cross > 0 and slope_20 > 0 and slope_50 > 0 and strength_ok:
        return +1, f"Trend ↑  E20>E50 ({pct:+.2f}%) ADX:{adx_current:.1f}"
    if cross < 0 and slope_20 < 0 and slope_50 < 0 and strength_ok:
        return -1, f"Trend ↓  E20<E50 ({pct:+.2f}%) ADX:{adx_current:.1f}"
    return 0, f"Trend →  karışık ({pct:+.2f}%) ADX:{adx_current:.1f}"
...
def algo_mr(klines: list[dict]) -> tuple[int, str]:
    ...
    vwap_vote = +1 if price < vwap_current * 0.998 else -1 if price > vwap_current * 1.002 else 0

    score = rsi_vote + bb_vote + vwap_vote
    vote = +1 if score >= 2 else -1 if score <= -2 else 0

    arr = "↑" if vote > 0 else "↓" if vote < 0 else "→"
    return vote, f"MR {arr}  RSI:{rsi_current:.1f}  BB_pos:{bb_position:.2f}  VWAP:{vwap_current:,.2f}"
...
def algo_orderflow_v2(klines: list[dict], ob: dict) -> tuple[int, str]:
    ...
    arr = "↑" if vote > 0 else "↓" if vote < 0 else "→"
    return vote, f"OF{tag} {arr}  CVD:{cvd_r:+.3f}  OB:{ob_r:+.3f}  Spread:{spread:.4f}%"
...
def algo_volume(klines: list[dict]) -> tuple[int, str]:
    ...
    if vol_ratio >= VOLUME_MIN_RATIO and vol_trend >= 3:
        if price_change > 0:
            vote = +1
        elif price_change < 0:
            vote = -1
    ...
    arr = "↑" if vote > 0 else "↓" if vote < 0 else "→"
    return vote, f"Vol {arr}  ratio:{vol_ratio:.2f}x  trend:{vol_trend}/5  Δ%:{price_change:+.2f}"
...
def calculate_position_size(direction: str, atr_val: float, price: float,
                            portfolio_value: float = DEFAULT_PORTFOLIO) -> float:
    ...
    position_size = min(position_size, max_position, BASE_TRADE_AMOUNT * 2)
    position_size = max(position_size, BASE_TRADE_AMOUNT * 0.5)
...
def analyze(
    ...
    *,
    ...
    try:
        ...
        print(f"[btc_5m_104_algo] Veri hatası: {e}", flush=True)
        return None
    ...
    htf_data = get_htf_trend(SYMBOL) if htf_confirmation else {"direction": "NEUTRAL", "strength": 25.0}
    ...
    if htf_confirmation and htf_data["direction"] != "NEUTRAL":
        if direction == "DOWN" and htf_data["direction"] == "UP" and htf_data["strength"] > 25:
            return SignalResult(
                ...
                htf_trend=htf_data["direction"], htf_strength=htf_data["strength"],
                raw_direction="DOWN",
                skip_reason=f"HTF onay: 1H trend UP (ADX:{htf_data['strength']:.1f}), DOWN engellendi",
            )
        if direction == "UP" and htf_data["direction"] == "DOWN" and htf_data["strength"] > 25:
            return SignalResult(
                ...
                htf_trend=htf_data["direction"], htf_strength=htf_data["strength"],
                raw_direction="UP",
                skip_reason=f"HTF onay: 1H trend DOWN (ADX:{htf_data['strength']:.1f}), UP engellendi",
            )
...
def format_signal(result: SignalResult | None) -> str:
    ...
    if result.direction is None:
        ...
        return (
            ...
            f"   HTF: {result.htf_trend} (ADX:{result.htf_strength:.1f}) | Vol: {result.volume_ratio:.2f}x"
        )
...
if __name__ == "__main__":
    print("=" * 60)
    print("BTC/USDT 5M Gelişmiş Sinyal Motoru v2.0 — 104")
    print("=" * 60)
