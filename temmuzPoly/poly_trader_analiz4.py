...
async def run_open() -> None:
    ...
    _at_risk4 = sum(p.get("amount", SANAL_TRADE_AMOUNT) for p in state["open_positions"])
...
def run_weekly() -> None:
    ...
    for d in range(7):
        for h in range(24):
            if not np.isnan(rate[d][h]):
                ...
                ax.text(h, d, f"%{pct}{'⚠' if n < MIN_STAT_COUNT else ''}\n+{grid_w[d][h]}-{n-grid_w[d][h]}/{n}",
                        ha="center", va="center", fontsize=5.5, color=clr, fontweight="bold", linespacing=1.3)
    ...
    ax.set_title(
        f"4. ANALİZ — Trend + MR + Orderflow + Funding  ({now_tr.strftime('%d.%m.%Y %H:%M İST')})\n"
    ...
    tg_send_photo(WEEKLY_IMG, f"⚡ 4. ANALİZ Haftalık Rapor — {now_tr.strftime('%d.%m.%Y %H:%M İST')}\n"
