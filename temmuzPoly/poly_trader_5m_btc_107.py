"""
5M 107 BTC — RSI Div (#5) Sanal Trader
=======================================
Algo #5 (RSI + Divergence) tek başına — UP/DOWN sinyali varsa işlem.
...
def rsi_div_signal(klines: list[dict]) -> tuple[str | None, str]:
    sig = A.rsi_div(_to_algo_kl(klines))
    if sig not in ("UP", "DOWN"):
        return None, f"#5 RSI Div: {sig} → nötr"
    return sig, f"#5 RSI Div: {sig}"
...
def analyze() -> dict | None:
    ...
    direction, detail = rsi_div_signal(klines)
    ...
    if direction is None:
        return {
            "direction": None,
            "consensus": 0,
            "sig5": detail.split(":")[1].split("→")[0].strip() if ":" in detail else "NEUTRAL",
            "detail": detail,
...
def run() -> None:
    ...
    if direction is None:
        tg_send(
            ...
            f"#5 RSI Div nötr → işlem açılmadı\n"
            f"📊 {detail}\n"
    ...
    token_price = pm_info["up_price"] if pm_info and direction == "UP" else (
        pm_info["down_price"] if pm_info else None
    )
    ...
    tg_send(
        ...
        f"{dir_icon} <b>BTC {dir_tr}</b>  (#5 RSI Div)  💵 ${amount:.0f} {price_str} → 🏆 ${to_win:.2f}\n"
    ...
    print(f"[{LABEL}] {saat} — {dir_tr}  ${amount:.0f}→${to_win:.2f}  entry:{entry_p:,.2f}  #5={sig5}")
