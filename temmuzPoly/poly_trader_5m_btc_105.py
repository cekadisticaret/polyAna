...
btc_5m_105_algo: 4-algo konsensüs + momentum + iki düzeltme:
  1. MR -1 iken UP için 3/4 gerekir
  2. Trend nötr band (zayıf E20/E50 cross/slope → 0 oy)
...
def run() -> None:
    ...
    amount    = sig.amount or (TRADE_AMOUNT_UP if direction == "UP" else TRADE_AMOUNT_DOWN)
    if balance < amount:
        tg_send(
            f"{sep}\n⏸ <b>{LABEL} — {saat} İST</b>\n"
    ...
    print(f"[{LABEL}] {saat} — {dir_tr} ${amount:.0f}→${to_win:.2f}  {format_signal(sig).split(chr(10))[0]}")
