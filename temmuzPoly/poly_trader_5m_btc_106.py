...
def _pm_resolve_market(symbol: str, ts_5m: int, direction: str, amount: float) -> tuple[dict | None, str]:
    ...
    if est < amount * _PM_MIN_PAYOUT_RATIO:
        return None, f"payout {est/amount:.2f}x düşük"
    pm["token_price"] = tp
    return pm, ""
...
def run() -> None:
    ...
    if state["open_positions"]:
        ...
        for pos in list(state["open_positions"]):
            ...
            spent = pos.get("pm_spent") or pos.get("amount", _trade_amount(pred))
    ...
    for sym in SYMBOLS:
        ...
        try:
            ...
            save_signal(f"106_{name.lower()}", ts_5m, sig.to_dict() if sig else None)
        except Exception as e:
        ...
        open_lines.append(
            ...
            f"💵 ${amount:.0f} @{token_price:.2f} → 🏆 ${to_win:.2f}\n"
            f"   Giriş: {_fmt_price(sym, sig.entry_price)}  mom:{sig.momentum}\n"
            f"   {icons}"
        )
        print(f"[{LABEL}] {saat} — {name} {dir_tr} ${amount:.0f}→${to_win:.2f}")
...
def _send_tg_round(
    ...
    if closed_lines:
        ...
        display_pnl = round(state["balance"] - INITIAL_BALANCE, 2)
        parts.append(
