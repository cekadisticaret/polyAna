"""
5M 106 BTC — RSI Div + Markov Sanal Trader
==========================================
Algo #5 (RSI + Divergence) + #35 (Markov Zinciri) — ikisi hemfikir olunca işlem.
...
STATE_FILE   = os.path.join(_DIR, "poly_trader_5m_btc_106_state.json")
HISTORY_FILE = os.path.join(_DIR, "poly_trader_5m_btc_106_history.json")
WEEKLY_IMG   = "/tmp/poly_5m_btc_106_weekly.png"
...
def _to_algo_kl(klines: list[dict]) -> list[dict]:
...
def dual_algo_signal(klines: list[dict]) -> tuple[str | None, str, str, str]:
    """Returns: (direction or None, sig5, sig35, detail)"""
    akl = _to_algo_kl(klines)
    sig5  = A.rsi_div(akl)
    sig35 = A.markov_chain(akl)

    if sig5 not in ("UP", "DOWN") or sig35 not in ("UP", "DOWN"):
        detail = f"#5 RSI Div: {sig5}  |  #35 Markov: {sig35} → uyumsuz/nötr"
        return None, sig5, sig35, detail

    if sig5 != sig35:
        detail = f"#5 RSI Div: {sig5}  |  #35 Markov: {sig35} → çelişki"
        return None, sig5, sig35, detail

    detail = f"#5 RSI Div: {sig5}  +  #35 Markov: {sig35} → hemfikir"
    return sig5, sig5, sig35, detail
...
def analyze() -> dict | None:
    ...
    direction, sig5, sig35, detail = dual_algo_signal(klines)
...
def run() -> None:
    ...
    sig5      = result.get("sig5")
    sig35     = result.get("sig35")
    ...
    if direction is None:
        tg_send(
            ...
            f"#5+#35 hemfikir değil → işlem açılmadı\n"
            f"📊 {detail}\n"
            f"💰 Bakiye: ${state['balance']:.2f}\n"
            f"{sep}"
        )
        print(f"[{LABEL}] {saat} — sinyal yok ({sig5}/{sig35})")
        return
    ...
    tg_send(
        ...
        f"{dir_icon} <b>BTC {dir_tr}</b>  (2/2 #5+#35)  💵 ${amount:.0f} {price_str} → 🏆 ${to_win:.2f}\n"
    ...
    print(f"[{LABEL}] {saat} — {dir_tr}  ${amount:.0f}→${to_win:.2f}  entry:{entry_p:,.2f}  #5={sig5} #35={sig35}")
