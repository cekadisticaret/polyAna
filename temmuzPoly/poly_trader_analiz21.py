"""
Poly Sanal Trader — 21. Analiz (Sembol bazlı en iyi algo)
Modlar:
...
STATE_FILE   = os.path.join(_DIR, "poly_trader_analiz21_state.json")
...
async def _fetch_klines(symbol: str, limit: int = 3) -> list:
    return await asyncio.to_thread(fetch_klines, symbol, "1h", limit)
...
async def run_close() -> None:
    ...
    for pos in list(state["open_positions"]):
        klines = await _fetch_klines(pos["symbol"], 2)
        if not klines:
        ...
        algo    = pos.get("algo_name", "")
        lines.append(f"{icon} {name}  {pred}  {entry:.2f} → {current_price:.2f} ({pct:+.2f}%)  {pnl_str}  [{algo}]")
...
async def run_open() -> None:
    ...
    for sym in SYMBOLS:
        ...
        if pred_obj is None or not pred_obj.predicted_dir:
            algo = pred_obj.algo_name if pred_obj else "?"
            print(f"[{_LABEL} open] {sym} — sinyal yok (BEKLE) [{algo}]")
            continue

        dyn_amount = _trade_amount(history, sym)
        try:
            klines = await _fetch_klines(sym, 3)
            entry_price = klines[-2]["c"] if klines and len(klines) >= 2 else pred_obj.current_price
        except Exception:
...
def run_weekly() -> None:
    ...
    tg_send_photo(WEEKLY_IMG, f"📊 {_LABEL} Haftalık — {genel} | ${balance:.2f}")
    print(f"[{_LABEL} weekly] gönderildi — {total} işlem")
...
def run_stats() -> None:
    ...
    for sym in SYMBOLS:
        ...
        if s:
            parts.append(f"📌 {name}: {_wr(sum(1 for t in s if t['win']), len(s))}")
    tg_send("\n".join(parts))
