...
STATE_FILE   = os.path.join(_DIR, "poly_trader_analiz9_state.json")
...
def _pm_get_client():
    ...
    for attempt in range(3):
        try:
            ...
            print(f"[9. ANALİZ] Client init ({attempt+1}/3): {e}", file=sys.stderr)
            time.sleep(2)
...
def _pm_find_market(symbol: str, et_hour: int, date_utc) -> dict | None:
    ...
    for slug in slugs:
        try:
            ...
            print(f"[9. ANALİZ] Gamma hatası ({slug}): {e}", file=sys.stderr)
    return None
...
def _pm_place_order(token_id: str, amount_usd: float, tick_size: str = "0.01",
    ...
    try:
        ...
        if not resp or not resp.get("success"):
            print(f"[9. ANALİZ] Order başarısız: {resp}", file=sys.stderr)
            return None
        oid = resp.get("orderID") or resp.get("id", "")
        return {"order_id": oid, "size": size, "price": price, "spent": spent}
    except Exception as e:
        print(f"[9. ANALİZ] Order hatası: {e}", file=sys.stderr)
        return None
...
def analyze(symbol: str) -> dict | None:
    try:
        ...
        print(f"[9. ANALİZ] {symbol} veri hatası: {e}", file=sys.stderr)
        return None
...
async def run_close() -> None:
    ...
    if not state["open_positions"]:
        tg_send(f"⏸ <b>9. ANALİZ — {saat} İST</b>\nKapatılacak açık pozisyon yok.")
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0:
                    ...
                    print(f"[9. ANALİZ close] {pos['symbol']} fiyat hatası: {e}", file=sys.stderr)
    ...
    if failed_pos:
        ...
        tg_send(f"⚠️ <b>9. ANALİZ</b> — {names} fiyatı alınamadı (timeout), bir sonraki saate bırakıldı.")
    ...
    tg_send(
        f"{sep}\n"
        f"🏁 <b>9. ANALİZ — {int(saat[:2]):02d}:00 Sonuçlar</b>\n"
        + "\n".join(lines) + "\n"
...
async def run_open() -> None:
    ...
    parts = [sep, f"🆕 <b>9. ANALİZ — {saat} - {next_h} Yeni İşlemler</b>"]
...
def run_weekly() -> None:
    ...
    ax.set_title(
        f"9. ANALİZ — Trend + MR + Orderflow + Funding  ({now_tr.strftime('%d.%m.%Y %H:%M İST')})\n"
    ...
    tg_send_photo(WEEKLY_IMG, f"⚡ 9. ANALİZ Haftalık Rapor — {now_tr.strftime('%d.%m.%Y %H:%M İST')}\n"
                              f"{total} işlem | {genel} | Trend+MR+OF+Funding")

    ind_lines = _ind_stats_lines(history)
    tg_send(
        f"📊 <b>9. ANALİZ HAFTALIK İSTATİSTİKLER</b>\n"
    ...
    print(f"[9. ANALİZ weekly] gönderildi — {total} işlem")
...
def run_stats() -> None:
    ...
    tg_send("\n".join(parts))
    print("[9. ANALİZ stats] gönderildi")
