"""
6. ANALİZ — MACD Hist. Div sanal trader

Modlar:
  close   → :02 — önceki saatin sonuçlarını kapatır
  open    → :05 — MACD Histogram Divergence (#26) sinyali ile işlem açar
...
Algoritma: algo_signals.macd_histogram_div (MACD Hist. Div #26)
Sanal bütçe: $300, işlem $12/$16/$20 (sembol WR — 1. Analiz mantığı).
Semboller: BTC + SOL. NEUTRAL sinyalde işlem yok.
...
async def _resolve_signal(symbol: str) -> tuple[str | None, float | None]:
    """MACD Hist. Div → UP/DOWN/None (NEUTRAL)."""
    try:
...
async def run_open() -> None:
    ...
    # MACD Hist. Div sinyali
    candidates = []
    for sym in SYMBOLS:
        direction, price = await _resolve_signal(sym)
        if direction is None:
            if price is not None:
                print(f"[{LABEL} open] {sym} — NEUTRAL, işlem yok")
            continue
        candidates.append({"sym": sym, "direction": direction, "price": price})
...
def _ind_stats_lines(history: list) -> list[str]:
    vals = [t["algo_ok"] for t in history if t.get("algo_ok") is not None]
    if vals:
        w = sum(1 for v in vals if v)
        n = len(vals)
        bar = "🟢" if w / n >= 0.6 else "🟡" if w / n >= 0.5 else "🔴"
        return [f"  {bar} {ALGO_NAME}: {_wr(w, n)}"]
    return [f"  ⚪ {ALGO_NAME}: veri yok"]
