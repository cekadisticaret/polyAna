"""2.Poly — A2#05 Mean Reversion sanal trader (temmuzPoly bağımsız)."""
...
_SIGNALS_FILE = os.path.join(_DIR, "signals.json")

# A2 Top 17 — sanal analizler (8727030715) botundan AYRI kanal/bot
BOT_TOKEN = os.getenv("TELEGRAM_A2_BOT_TOKEN", "")
CHAT_ID = os.getenv("TELEGRAM_A2_CHAT_ID", "")
_A2_ALFA_CHANNEL_ALGOS = frozenset()
_ALFA_BOT_TOKEN = os.getenv("TELEGRAM_ALFA_BOT_TOKEN", "")
_ALFA_CHAT_ID = os.getenv("TELEGRAM_ALFA_CHAT_ID", "")
...
async def run_open(cfg: A2Config, *, notify: bool = True) -> str | None:
    now_tr = datetime.now(timezone.utc).astimezone(_TZ_TR)
    from balance_guard import is_a2_live_dashboard_open
    ...
    for c in candidates:
        ...
        already = any(
            p.get("symbol") == sym and p.get("entry_hour_tr") == hour_tr
            for p in state.get("open_positions") or []
        )
        if already:
            print(f"[{cfg.label} open] {sym} — bu saat zaten açık")
            continue
        ...
        # Live mirror yalnız run_live.py / cron — sanal open otomatik PM açmaz
        opened.append({
            **c,
            "pos": pos,
...
PaperConfig = A2Config


def disable_telegram() -> None:
    global tg_send_for_cfg, tg_send
    tg_send_for_cfg = lambda _cfg, _text: None  # type: ignore[assignment]
    tg_send = lambda _text: None  # type: ignore[assignment]
