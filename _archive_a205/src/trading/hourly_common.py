...
def try_pm_open(
    ...
    *,
    ...
    # Aynı market+token'da zaten açık pozisyon varsa ikinci kez girme:
    # zincir uzlaşması market bazlı olduğu için mükerrer giriş P&L'i bozar.
    for p in state.get("open_positions") or []:
        if p.get("pm_slug") == pm["slug"] and p.get("pm_token_id") == token_id:
            pm_log_hata(hata_file, sym, "duplicate_market", f"slug={pm['slug']}")
            return None, "duplicate_market"
