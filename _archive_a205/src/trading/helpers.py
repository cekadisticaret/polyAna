...
PM_ORDER_ATTEMPTS = 3
# Emir sonrası zincir indeksinin oturması için bekleme (mükerrer dolum koruması)
PM_ORDER_SETTLE_SEC = 4
...
def _weekend_pause_applies(label: str) -> bool:
    ...
    if label.startswith("15M"):
        return True
    try:
        from balance_guard import is_live_pm_label
...
def skip_if_weekend_pause(
    ...
    try:
        from balance_guard import skip_algo_islemler_open_deferred
    ...
    try:
        from balance_guard import is_dashboard_live_open
...
def load_sanal_wr_amounts(book_key: str) -> tuple[float, float, float]:
    """Poly sanal defter — sembol WR giriş kademeleri (trade_settings.json)."""
...
_PM_LIVE_SETTINGS_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "config", "trade_settings.json"
)
...
def load_pm_live_amounts(system: str) -> tuple[float, float, float]:
    """Dashboard trade_settings.json → (low, mid, high) gerçek PM giriş tutarları."""
...
def save_pm_live_amounts(
    system: str, low: float, mid: float, high: float,
) -> tuple[float, float, float]:
    """Gerçek PM stake kademelerini kaydet (dashboard)."""
    low = round(max(0.5, min(500.0, float(low))), 2)
    mid = round(max(0.5, min(500.0, float(mid))), 2)
    high = round(max(0.5, min(500.0, float(high))), 2)
    # mid düşük/yüksek arasında kalsın
    if mid < low:
        mid = low
    if high < mid:
        high = mid
    ...
    if os.path.exists(_PM_LIVE_SETTINGS_FILE):
        try:
            ...
            data = {}
    data[f"{system}_amount_low"] = low
    data[f"{system}_amount_mid"] = mid
    data[f"{system}_amount_high"] = high
    with open(_PM_LIVE_SETTINGS_FILE, "w") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    return low, mid, high
    ...
    for attempt in range(3):
        try:
            ...
            if creds is None:
                time.sleep(0.5)
                continue
            sig_type = int(os.getenv("POLY_SIGNATURE_TYPE", "3") or "3")
            client = ClobClient(
...
def pm_place_order(
    ...
    while attempt < limit:
        attempt += 1
        # Önceki deneme yanıt vermemiş ama zincirde dolmuş olabilir. Yeni emir
        # göndermeden önce tazele — yoksa aynı pozisyon için ikinci kez para gider.
        if attempt > 1:
            already = _pm_recover_filled_order(
        ...
        # Zincir indeksi birkaç saniye gecikebiliyor; hemen sorarsak dolumu
        # göremeyip tekrar emir gönderiyorduk.
        time.sleep(PM_ORDER_SETTLE_SEC)
...
def pm_chain_settlement(slug: str, token_id: str = "") -> dict | None:
    """Zincirdeki gerçek hesap: ücret dahil harcama, gelen para, kalan hisse.

    Bot'un kendi tuttuğu tutar taker ücretini içermiyor; ayrıca pozisyon
    arayüzden elle satılmışsa ödeme hiç gelmiyor. Tek doğru kaynak zincir.
    """
    import urllib.request

    funder = os.getenv("POLY_FUNDER", "")
    if not funder or not slug:
        return None
    try:
        req = urllib.request.Request(
            f"https://data-api.polymarket.com/activity?user={funder}&limit=200",
            headers={"User-Agent": "2.Poly", "Accept": "application/json"},
        )
        with urllib.request.urlopen(req, timeout=15) as r:
    ...
    for a in rows:
        if a.get("slug") != slug:
            continue
        # Aynı markette hem UP hem DOWN tutuluyorsa slug bazlı toplam iki tarafı
        # birbirine karıştırır; token varsa ona göre daralt.
        if token_id and str(a.get("asset") or "") not in ("", str(token_id)):
            continue
    ...
    shares = pm_conditional_shares(token_id) if token_id else -1.0
    if shares < 0:
        return None
    return {"spent": round(spent, 2), "proceeds": round(proceeds, 2), "shares": shares}
