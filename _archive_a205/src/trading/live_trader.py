...
from hourly_common import tg_send, try_pm_open
from klines import _fetch_klines
from paper_trader import _sym_short
from helpers import (
...
from balance_guard import can_open_trade, is_dashboard_live_open
...
def _paths(spec: A2LiveSpec) -> tuple[str, str, str]:
    return (
        os.path.join(_DIR, "live_state.json"),
        os.path.join(_DIR, "live_history.json"),
        os.path.join(_DIR, "live_errors.json"),
    )
...
def _paper_state_path(algo_num: int) -> str:
    return os.path.join(_DIR, "paper_state.json")


def _live_has_paper_pos(live_state: dict, paper_pos: dict) -> bool:
    sym = paper_pos.get("symbol")
    hour = paper_pos.get("entry_hour_tr")
    slug = paper_pos.get("pm_slug")
    for p in live_state.get("open_positions") or []:
        if p.get("symbol") != sym:
            continue
        if hour is not None and p.get("entry_hour_tr") == hour:
            return True
        if slug and p.get("pm_slug") == slug and p.get("entry_hour_tr") == hour:
            return True
    return False
...
async def run_close(spec: A2LiveSpec) -> None:
    ...
    tur_pnl = 0.0
    failed = []
    # Aynı market+token'da birden fazla alım varsa zincir toplamı ortaktır;
    # her pozisyona ayrı ayrı yazılırsa P&L kat kat şişer.
    slug_token_count: dict[tuple, int] = {}
    for p in state["open_positions"]:
        k = (p.get("pm_slug"), p.get("pm_token_id"))
        slug_token_count[k] = slug_token_count.get(k, 0) + 1
    ...
    for pos in list(state["open_positions"]):
        ...
        if has_pm:
            ...
            shared = slug_token_count.get((pos.get("pm_slug"), pos.get("pm_token_id")), 1) > 1
            chain = (
                None if shared
                else pm_chain_settlement(pos.get("pm_slug", ""), pos.get("pm_token_id", ""))
            )
            if chain:
...
async def mirror_open_from_sanal(
    ...
    *,
    ...
    state = load_state(spec)
    if _live_has_paper_pos(state, sanal_pos):
        return None, "exists"
...
async def sync_open_from_sanal_state(spec: A2LiveSpec) -> list[tuple[str, str, float, dict]]:
    """Paper defterindeki bu saat slotundaki pozisyonları live'a yansıt."""
    if not is_live_active(spec):
        print(f"[{spec.label} sync] live kapalı — atlandı")
        return []

    spath = _paper_state_path(spec.algo_num)
    ...
    for sanal_pos in sanal_state.get("open_positions") or []:
        if int(sanal_pos.get("entry_hour_tr", -1)) != hour_tr:
            continue
...
async def run_open(spec: A2LiveSpec) -> None:
    """Live open — paper defterinden yansıt (sinyal dosyasından doğrudan açma yok)."""
    now_tr = datetime.now(timezone.utc).astimezone(_TZ_TR)
