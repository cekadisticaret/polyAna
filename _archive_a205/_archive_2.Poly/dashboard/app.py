...
def _format_pos(
    ...
    # Bot açılışta niyet ettiği rakamı yazıyor; zincirdeki gerçekleşen dolum farklı
    real = (chain or {}).get(pos.get("pm_slug") or "")
    if real:
        spent = real["spent"]
        size = real["size"]
...
def build_a205() -> dict:
    ...
    from pm_balance_guard import get_pm_system_control, get_usdc_balance, is_group_paused

    now_tr = datetime.now(timezone.utc).astimezone(TZ_TR)
    settings = _read_json(ROOT / "config" / "analiz5_settings.json", {})

    sanal_state = _read_json(DATA / "poly_trader_a2_05_state.json", {})
    sanal_history = _read_json(DATA / "poly_trader_a2_05_history.json", [])
    live_state = _read_json(DATA / "poly_trader_a2_05_live_state.json", {})
    live_history = _read_json(DATA / "poly_trader_a2_05_live_history.json", [])
    signals_raw = _read_json(DATA / "algo_signals_v2.json", {})
    control = get_pm_system_control()
    ...
    open_positions = [
        _format_pos(p, "live", spot, chain.get("per_slug")) for p in live_open
    ]
    ...
    return {
        ...
        "live": {
            **live_stats,
            ...
            "pnl": round(float(live_state.get("total_pnl") or live_stats["pnl"]), 2),
        },
        ...
        "cron": [
            {"time": ":02", "label": "Live kapat"},
            {"time": ":04:30", "label": "Sinyal üret"},
            {"time": ":05:00", "label": "Live PM aç"},
            {"time": ":05:10", "label": "Açılış retry"},
        ],
        "mode": "live_only",
        "control": {
...
@app.get("/")
def index():
    return send_from_directory(DASHBOARD_DIR / "static", "a205.html")
...
def api_a205_amounts():
    from pm_trader_helpers import save_pm_live_amounts
...
def toggle_live():
    from pm_balance_guard import toggle_group_paused
...
def api_redeem():
    ...
    from pm_redeem import redeem_all
