...
def _usd(entry: float, dist: float) -> float:
    """Fiyat mesafesini $ karşılığına çevirir ($1 ≈ $11.2 @ 500x)."""
    return dist / float(entry) * MARGIN * LEVERAGE


def _pnl(side: str, entry: float, exit_px: float) -> float:
    sign = 1.0 if side == "buy" else -1.0
    return sign * _usd(entry, exit_px - entry)
...
def _float_pnl(pos: dict, bid: float | None, ask: float | None) -> float | None:
    ...
    if not pos or bid is None or ask is None:
        return None
    return round(_pnl(pos["side"], pos["entry"], _exit_px(pos["side"], bid, ask)), 2)
...
def _net_float(pos: dict, bid: float | None, ask: float | None) -> float | None:
    ...
    if fp is None:
        return None
    return round(fp - _commission_side(), 2)
...
def _plan(side: str, entry: float, levels: dict | None) -> dict | None:
    ...
    if side == "buy":
        ...
        reward = _usd(entry, (res - _LEVEL_PAD) - entry)
        risk = _usd(entry, entry - stop)
        kind = "Direnç"
    else:
        if sup >= entry or res <= entry:
            return None
        target, stop = sup, round(res + _STOP_PAD, 2)
        reward = _usd(entry, entry - (sup + _LEVEL_PAD))
        risk = _usd(entry, stop - entry)
        kind = "Destek"
...
def _px_of_usd(entry: float, usd: float) -> float:
    return float(usd) / (MARGIN * LEVERAGE) * float(entry)
...
def _update_lock(pos: dict, mark: float) -> None:
    ...
    peak_usd = max(float(pos.get("peak_usd") or 0.0), _pnl(pos["side"], entry, mark))
    ...
    if peak_usd >= LOCK_TRAIL_USD:
        keep_px = _px_of_usd(entry, peak_usd * 0.5)
        stage = 2
...
def _accrue_swap(st: dict, pos: dict) -> bool:
    """00:00 İST rollover. Çarşamba ×3; Cmt/Paz atlanır."""
...
def _close_one(st: dict, hist: list, pos: dict, bid: float, ask: float, reason: str, book: str = "g1") -> dict | None:
    ...
    gross = round(_pnl(pos["side"], pos["entry"], exit_px), 2)
    comm_close = _commission_side()
...
def _protect(st: dict, hist: list, bid: float, ask: float, rail=None, levels=None, book: str = "g1") -> bool:
    ...
    for pos in list(_plist(st)):
        if (pos.get("target") is None or pos.get("stop") is None) and levels:
            plan = _plan(pos["side"], float(pos["entry"]), levels)
            if plan:
...
def _open(st: dict, side: str, bid: float, ask: float, signal: str, plan: dict) -> dict | None:
    rows = _plist(st)
...
def apply_signal(
    ...
    with open(lock_p, "a+", encoding="utf-8") as lk:
        ...
        if want and not _has_side(st, want):
            ...
            plan = None if wait else _plan(want, _open_px(want, bid, ask), levels)
            ...
            elif _open(st, want, bid, ask, direction, plan):
                st["last_reject"] = None
                dirty = True
...
def snapshot(bid: float | None = None, ask: float | None = None, book: str = "g1") -> dict:
    ...
    return {
        ...
        "costs": {
            "commission_side": _commission_side(),
            "commission_rt": round(_commission_side() * 2, 2),
