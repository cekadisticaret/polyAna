...
def overlay_signals(tf: str, candles: list[dict]) -> tuple[dict, list[dict]]:
    ...
    packed = _apply_rail_veto(packed, _rail_now())
...
def latest_signal(tf: str, candles: list[dict] | None = None) -> dict:
    """Ray / teyit — Kalman+VWAP, tick yok (döngü olmasın)."""
    from forex_data import get_xau_klines

    if candles is None:
        candles, _ = get_xau_klines(tf, 120)
    if len(candles) < 30:
        return _neutral()

    htf_rows, _ = get_xau_klines(_HTF.get(tf, "1h"), 80)
    htf_df = _to_df(htf_rows) if htf_rows else None
    df = _to_df(candles)
    engine = SignalEngine(_RAIL_CFG)
    last: SignalResult | None = None
...
def live_signal(tf: str, candles: list[dict] | None = None) -> dict:
    ...
    return _apply_rail_veto(packed, _rail_now())
...
def rail_signals() -> dict:
    """Grafik sol şeridi — M5 ve M15 yönü (artacak / düşecek)."""
    from forex_data import bar_remaining, get_xau_klines

    out: dict[str, dict] = {}
    for tf, lab in (("5m", "M5"), ("15m", "M15")):
        rows, _ = get_xau_klines(tf, 120)
        sig = latest_signal(tf, rows)
        # Kapanmış mumun yönü — defter çıkışı oluşan mumun salınımıyla tetiklenmesin
        closed = latest_signal(tf, rows[:-1]) if len(rows) > 31 else sig
