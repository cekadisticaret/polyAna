...
_UA = {"User-Agent": "aiProject-bin-b103/1.0"}
_INFO_TTL = 3600.0
_DEPTH_TTL = 2.0
_STATUS_TTL = 8.0
...
_CONTROL = _DIR / "data" / "bin_b103_live_control.json"
...
def place_market(
    ...
    *,
    ...
    """XAUUSDT Isolated MARKET — GPSUSDT / CR6 state'e yazmaz."""
    ...
    try:
        ...
        print(
            f"[BIN_B1#03] LIVE {side_u} qty={exe} @{avg} "
...
def close_live(*, fallback_px: float = 0.0, attempts: int = 4) -> dict:
    ...
    for i in range(max(1, int(attempts))):
        ...
        fees += float(fee or 0)
        print(
            f"[BIN_B1#03] LIVE CLOSE {side_u} qty={exe} @{avg} "
