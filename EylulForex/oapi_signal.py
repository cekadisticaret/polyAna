"""OPEN API sinyal — CAPITAL kopyası. forex_signal/cem02_signal import etmez."""
...
_SHADOW = str(_DIR / "oapi_shadow_signals.jsonl")
...
def _tick_payload() -> dict:
    try:
        from oapi_data import paxg_tick_score
...
def _rail_now() -> dict:
    try:
        from oapi_data import forex_rail
        return forex_rail()
    except Exception:
        return {}
...
def _fetch_rows(tf: str, n: int, klines_fn=None) -> list[dict]:
    ...
    from oapi_data import get_xau_klines
...
def rail_signals(klines_fn=None) -> dict:
    ...
    from oapi_data import bar_remaining
