"""OPEN API veri — CAPITAL kopyası. forex_data/cem02_data import etmez."""
...
def get_xau_klines(tf: str = "1m", limit: int = 200) -> tuple[list[dict], str]:
    ...
    if hit and now - hit[0] < _CACHE_TTL:
        return hit[1], "cache"
    try:
        from ctrader_api import configured, prices as oapi_prices
        if configured():
            rows = oapi_prices(tf, n)
            if rows:
                _cache[key] = (now, rows)
                return rows, "ctrader"
    except Exception:
...
def forex_quote() -> dict:
    """Bid / ask / mid + günlük H/L. cTrader Open API varsa oradan."""
    global _quote_cache, _basis
    now = time.time()
    if _quote_cache and now - _quote_cache[0] < 2.0:
        return dict(_quote_cache[1])
    try:
        from ctrader_api import configured, quote as oapi_quote
        if configured():
            q = oapi_quote()
...
def forex_rail() -> dict:
    ...
    from oapi_signal import rail_signals
    data = rail_signals()
...
def forex_spot(timeframe: str = "1m", algo: str = "g1") -> dict:
    ...
    try:
        ...
        if configured():
            ...
            from oapi_book import apply_signal
            q["book"] = apply_signal(
