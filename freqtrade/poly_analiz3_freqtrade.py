"""
3. ANALİZ FREQTRADE — Saatlik Polymarket sanal (BTC + SOL + ETH)

Sinyal: freqtrade/user_data/strategies/SampleStrategy TA mantığı (analiz3_signal.py)
Sanal: $300, PM gamma kotasyonu, gerçek emir yok.
Freqtrade çekirdeğine dokunmaz; ayrı cron betiği.
...
_USERDATA = os.path.join(_DIR, "user_data")
STATE_FILE = os.path.join(_USERDATA, "analiz3_freqtrade_state.json")
HISTORY_FILE = os.path.join(_USERDATA, "analiz3_freqtrade_history.json")
...
async def run_open() -> None:
    ...
    for sym in SYMBOLS:
        ...
        pos = {
            ...
            "ind_rsi_vote": "UP" if sig.rsi < 50 else "DOWN",
            "ind_macd_vote": "UP" if sig.macd_bull else "DOWN",
            "ind_tema_vote": "UP" if sig.tema_rising else "DOWN",
            "ind_long_bias": sig.long_bias,
            "ind_short_bias": sig.short_bias,
        }
...
def run_stats() -> None:
    ...
    tg_send(
        ...
        f"BTC+SOL+ETH saatlik sanal PM · SampleStrategy TA"
    )
