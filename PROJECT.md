...
| `AgustosKripto/exit_policy.py` | **Çıkış rejimi tek kaynağı** — zaman kapanışı yok · 24s tavan · 6×ATR stop. Ölçüm, elenen alternatifler ve geri alma yolu dosyanın başında; `exit_policy.json` ile kod değişmeden kapatılır |
...
| `temmuzPoly/poly_trader_a2_05_v2.py` | **A2#05 V2 · FİYAT TABANI** sanal ($300) — A2#05 ile aynı sinyal (`algo_num=5`), tek fark **0,40 altı bileti alma** (`A2_05_V2_MIN_ENTRY_PRICE`). A2#05 dosyalarına dokunmaz, kendi state/history'si var. `live_mirror=False` → A2#05'in gerçek para aynasını tetiklemez; :02 close / :05 open / Cmt 21:00 weekly · URL `/algoritma-islemler/a2_05_v2` |
...
- **Çıkış rejimi TÜM sanal defterlerde değişti (2026-08-15, `exit_policy.py`):** Eski rejim `1h/4h zorunlu kapanış + 2×ATR zarar stopu` → yeni rejim **zaman kapanışı yok · 24s tavan · 6×ATR stop**. Algoritmalara ve girişlere dokunulmadı, yalnız çıkış kuralı değişti.
