...
| `AgustosKripto/crypto_futures_live_control.json` | Binance Live — `live_paused` (yeni açılış) + `top_n` (1–10) + `symbols` (beyaz liste, `FG_SYMBOLS` ile **kesişim**; boş kesişim = filtre yok sayılır); sanal Algoritma/Analiz etkilenmez |
...
| `AgustosKripto/kaito_paper.py` | **KAITO kağıt defteri** — cr6'nın 4 algo oyunu KAITO'da koşturur, gerçek emir yok; $7×20x, ATR kilidi + `exit_policy`, gerçek taker komisyonu; `open`/`close`/`trail`/`stats`; dashboard `/kripto` overview kartı + `/poly/api/kripto/kaito-paper` |
