...
WEEKLY_IMG    = "/tmp/coptc_poly_weekly_heatmap.png"
...
async def run_close() -> None:
    ...
    msg = (
        f"{sep}\n"
        f"🏁 <b>1. ANALİZ — {saat_round} Sonuçlar</b>\n"
        + "\n".join(lines) + "\n"
...
def run_weekly() -> None:
    ...
    caption = (
        f"📊 1. ANALİZ Haftalık Rapor — {now_tr.strftime('%d.%m.%Y %H:%M İST')}\n"
    ...
    pnl_icon  = "🟢" if total_pnl >= 0 else "🔴"
    tg_send(
        f"📊 <b>1. ANALİZ HAFTALIK İSTATİSTİKLER</b>\n"
