"""
B1#01 — Sembol bazlı en iyi algoritma birleşimi (algoritma-islemler)

Her sembol için A6/A6V2/A6V3/A15/A2#xx defterlerinden en yüksek WR motoru.
...
from b1_01_signal import SYMBOLS, resolve_live_signal, engine_label, load_mapping
...
STATE_FILE    = os.path.join(_DIR, "poly_trader_b1_01_state.json")
HISTORY_FILE  = os.path.join(_DIR, "poly_trader_b1_01_history.json")
WEEKLY_IMG    = "/tmp/coptc_poly_b1_01_weekly_heatmap.png"
LABEL         = "B1#01"
BOOK_KEY      = "b1_01"
ALGO_NAME     = "Sembol bazlı en iyi motor"
...
def tg_send(text: str) -> None:
    if os.getenv("COPTC_TELEGRAM", "off").strip().lower() not in ("on", "1", "true", "yes"):
        return   # CoptC: Telegram ana şalteri kapalı
    if not BOT_TOKEN:
        return   # CoptC: token tanımsız — Telegram kapalı
    try:
...
def run_preview() -> None:
    ...
    print(f"[{LABEL} preview] {now_tr.strftime('%H:%M')} İST — {next_hour:02d}:00 önizleme (TG atlanıyor)")
